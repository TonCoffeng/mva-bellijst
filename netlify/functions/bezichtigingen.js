  ...
  try {
    ...
    const bezichtigingen = rows.map(row => {
      ...
      return {
        ...
        datum,
        tijdstip,
        // DEBUG: tijdelijk meegestuurd om tijdzone-bug te diagnosticeren — haal weg na fix
        _debug_raw_datum_tijd: row.datum_tijd,
        _debug_iso_utc: row.datum_tijd ? new Date(row.datum_tijd).toISOString() : null,
        _debug_iso_local: row.datum_tijd ? new Date(row.datum_tijd).toString() : null,
