...
- **Actieve status-knop gemarkeerd** (CSS + render): de knop van de huidige lead-status (Warm/Hot/Afspraak/Deal/Lost) krijgt een duidelijk 3px gekleurd kader + opvulling in de statuskleur, zodat op de kaart in één oogopslag zichtbaar is waar de lead staat.
- **Dubbele-Deal-blokkade**: staat een lead al op Deal en klikt de makelaar nóg een keer op Deal, dan verschijnt de melding "Deze lead staat al op Deal — de deal is al geregistreerd" en stopt de functie. Voorkomt een tweede felicitatie, een tweede Cloze-actie en een dubbele €175-registratie. Een Deal-lead kan wel nog bewust naar een andere status (bv. terugdraaien naar Hot of Archief).
- **Kaart springt niet meer weg bij statuswijziging**: na een klik op Deal/Hot/Warm blijft de kaart op z'n plek staan (hersortering wordt één render overgeslagen via vlag `behoudVolgordeBijRender`), zodat de makelaar het gekleurde kader + de felicitatie meteen op de juiste plek ziet in plaats van dat de kaart naar onderaan de lijst verspringt. Bij de volgende verse render sorteert de lijst weer normaal.
